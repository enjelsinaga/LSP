...
.then(data => {
  // Ubah data dari format yang diterima ke format yang dibutuhkan oleh Chartist
  const formattedData = data.map(item => ({
    bulan: item.bulan,
    total_pendapatan: item.total_pendapatan / 1000, // Ubah ke format ribuan
  }));

  const chartData = {
    labels: formattedData.map(item => item.bulan),
    series: [formattedData.map(item => item.total_pendapatan)]
  };

  // Buat Chartist Line Chart
  new Chartist.Line('#chart-with-area', chartData, {
    low: 0,
    showArea: true,
    axisY: {
      scaleMinSpace: 50, // Menyesuaikan skala pada sumbu Y
      labelInterpolationFnc: function (value) {
        return value + 'K'; // Menambahkan label 'K' untuk ribuan
      }
    },
    plugins: [
      Chartist.plugins.tooltip({
        transformTooltipTextFnc: function(value) {
          return value + 'K'; // Menambahkan 'K' pada tooltip
        }
      })
    ]
  });
...
//Transaksi
  fetch('/dashboard/chartTransaksi') // Ganti dengan endpoint yang sesuai
  .then(response => response.json())
  .then(data => {
    const chartData = {
      labels: data.map(item => item.bulan),
      series: [data.map(item => item.jumlah_transaksi)] // Sesuaikan dengan atribut yang benar
    };

    // Buat Chartist Line Chart
    new Chartist.Line('#chart2-with-area', chartData, {
      low: 0,
      showArea: true,
      axisY: {
        scaleMinSpace: 50,
...
//   new Chartist.Line('#chart2-with-area', {
