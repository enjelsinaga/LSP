        ...
        <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4">
            ...
            <div class="row">
                    <div class="col-md-12">
                        ...
                        <a href="{{ route('data_pelajaran.index') }}" class="btn btn-secondary float-left ml-2">
                        ...
                        <form action="{{ route('data_pelajaran.update', $data->id) }}" method="post">
                            ...
                            <div class="form-group">
                                <label for="kelas_ajaran"><strong>Kelas Ajaran</strong></label>
                                <select class="form-control" name="kelas_ajaran">
                                    <option value="1 A" {{ $data->kelas_ajaran === '1 A' ? 'selected' : '' }}>1 A</option>
                                    <option value="1 B" {{ $data->kelas_ajaran === '1 B' ? 'selected' : '' }}>1 B</option>
                                    <option value="1 C" {{ $data->kelas_ajaran === '1 C' ? 'selected' : '' }}>1 C</option>
                                    <option value="2 A" {{ $data->kelas_ajaran === '2 A' ? 'selected' : '' }}>2 A</option>
                                    <option value="2 B" {{ $data->kelas_ajaran === '2 B' ? 'selected' : '' }}>2 B</option>
                                    <option value="2 C" {{ $data->kelas_ajaran === '2 C' ? 'selected' : '' }}>2 C</option>
                                    <option value="3 A" {{ $data->kelas_ajaran === '3 A' ? 'selected' : '' }}>3 A</option>
                                    <option value="3 B" {{ $data->kelas_ajaran === '3 B' ? 'selected' : '' }}>3 B</option>
                                    <option value="3 C" {{ $data->kelas_ajaran === '3 C' ? 'selected' : '' }}>3 C</option>
                                    <!-- Tambahkan opsi kelas lainnya sesuai kebutuhan -->
                                </select>
                                <div class="form-group">
                                <label class="font-weight-bold" for ="pengajar">Pengajar</label>
                                <?php $gurus = DB::select(DB::raw("select * from guru1")); ?>
                                <select name="pengajar" class="form-control @error('pengajar') is-invalid @enderror">
                                    @foreach ($gurus as $guru1)
                                        <option value="{{ $guru1->nama }}">{{ $guru1->nama }}</option>
                                    @endforeach
                                </select>
                                @error('guru1')
